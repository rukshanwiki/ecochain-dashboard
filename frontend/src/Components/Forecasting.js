...
// // Static daily demand to calculate comparisons against
// const dailyDemand = {
...
//     harvestingStart: "",  
//     harvestingEnd: "",  
//     harvestDuration: "7", // Added default harvest duration (days)
...
//   // --- Auto-Calculate Units & Default Duration ---
//   useEffect(() => {
...
//       // Calculate Harvest Start and End based on chosen duration
...
//         // Added dynamic harvest duration calculation
...
//   // --- ALTERNATIVE SUGGESTION ENGINE & FINANCIAL MODEL ---
//   let forecastSuggestion = null;
//   let financialImpact = null;

//   if (isForecasting) {
//       const baseProductData = baseDeclarations.filter(item => 
//           (item.product || "").trim().toLowerCase() === forecastForm.product.toLowerCase()
//       );
//       const currentSupply = baseProductData.reduce((sum, item) => sum + (parseFloat(item.units) || 0), 0);
//       const forecastLimit = consumptionLimits[forecastForm.product] || 1000;
//       const oversupplyThreshold = forecastLimit * 1.2;
      
//       const newUnits = parseFloat(forecastForm.units) || 0;
//       const harvestDays = parseInt(forecastForm.harvestDuration) || 7;
      
//       // Smart feature: Spreading harvest over more days lowers the weekly market shock!
//       const effectiveWeeklyUnits = newUnits / (harvestDays / 7);
//       const projectedTotal = currentSupply + effectiveWeeklyUnits;
...
//       const supplyRatio = projectedTotal / forecastLimit;
//       const marketElasticity = 1.2; 

//       let predictedPrice = historicalMonthlyPrice;
      
//       if (supplyRatio > 1.0) {
//           const priceDropPercentage = (supplyRatio - 1.0) * marketElasticity;
//           predictedPrice = Math.max(historicalMonthlyPrice * 0.3, historicalMonthlyPrice * (1 - priceDropPercentage)); 
//       }

//       const expectedRevenue = newUnits * historicalMonthlyPrice;
//       const predictedRevenue = newUnits * predictedPrice;
//       const estimatedLoss = expectedRevenue - predictedRevenue;
...
//           loss: Math.round(estimatedLoss).toLocaleString(),
//           isOversupplied: supplyRatio > 1.0
//       };

//       if (projectedTotal > oversupplyThreshold) {
//           const safeAdditionalUnits = Math.max(0, oversupplyThreshold - currentSupply);
...
//           const excessArea = (originalArea - safeArea).toFixed(2);
...
//                   alternatives: alternatives.length > 0 ? alternatives.join(", ") : "other local crops"
//               };
//           } else {
//               forecastSuggestion = {
//                   type: 'full',
...
//   let status = { color: "success", content: <span className="fs-5 fw-bold">Supply and demand are balanced ✅</span> };
...
//             <div className="fs-4 fw-bold mb-1">⚠ Oversupply Risk!</div>
//             <div className="fs-6 mb-3">Total Projected: {totalUnits} (Limit: {limit}). Prices are likely to drop.</div>
            
//             {/* 💸 FINANCIAL IMPACT WARNING */}
//             {financialImpact && financialImpact.loss !== "0" && financialImpact.isOversupplied && (
...
//                     <p className="mb-2">
//                       Historically, the price for {forecastForm.product} in <strong>{financialImpact.harvestMonth}</strong> is <strong>Rs {financialImpact.basePrice}/kg</strong>. <br/>
//                       However, due to the projected market flood, the price is predicted to crash to <strong>Rs {financialImpact.predictedPrice}/kg</strong>. 
//                       <br/><small className="text-muted">Hint: Try extending your 'Days to Harvest' to spread out supply and stabilize prices!</small>
//                     </p>
//                     <ul className="mb-0">
//                         <li>Expected Income (Normal Market): <strong>Rs {financialImpact.expectedRevenue}</strong></li>
//                         <li>Predicted Income (Oversupplied Market): <strong>Rs {financialImpact.predictedRevenue}</strong></li>
//                         <li className="text-dark fw-bold mt-1">Estimated Loss to You: Rs {financialImpact.loss}</li>
//                     </ul>
//                 </div>
//             )}

//             {/* Embedded Suggestion Card inside the Danger Alert */}
//             {forecastSuggestion && (
...
//                 <h5 className="fw-bold mb-3">💡 Smart Cultivation Suggestion for {forecastForm.locationName}:</h5>
//                 {forecastSuggestion.type === 'partial' ? (
//                   <p className="mb-0 fs-6" style={{ lineHeight: "1.6" }}>
//                     To protect your profits, consider reducing your <strong>{forecastForm.product}</strong> cultivation area to <strong>{forecastSuggestion.safeArea} Hectares</strong>. <br/>
//                     You can use the remaining <strong>{forecastSuggestion.excessArea} Hectares</strong> to cultivate alternative high-demand crops suitable for your region, such as: <strong className="text-success fs-5">{forecastSuggestion.alternatives}</strong>.
//                   </p>
//                 ) : (
//                   <p className="mb-0 fs-6" style={{ lineHeight: "1.6" }}>
//                     The market for <strong>{forecastForm.product}</strong> is heavily oversupplied. We strongly recommend dedicating your full <strong>{forecastForm.area} Hectares</strong> to alternative crops suitable for your region, such as: <strong className="text-success fs-5">{forecastSuggestion.alternatives}</strong>.
//                   </p>
//                 )}
//               </div>
//             )}
...
//     status = { color: "warning", content: <span className="fs-5 fw-bold">⚠ Near consumption margin ({totalUnits}/{limit}). Monitor closely.</span> };
//   }
...
//         if (chartHarvestMap[dStr]) {
//           weeklyHarvest += chartHarvestMap[dStr];
//         }
//       }

//       let weeklyDemand = 0;
//       if (chartVeg === "All") {
//         weeklyDemand = Object.values(dailyDemand).reduce((a, b) => a + b, 0) * 7;
//       } else {
//         weeklyDemand = (dailyDemand[chartVeg] || 0) * 7;
//       }

//       const formatStr = (d) => d.toLocaleDateString("en-US", { month: "short", day: "numeric" });
      
//       data.push({
//         weekRange: `${formatStr(wStart)} - ${formatStr(wEnd)}`,
//         Harvest: Math.round(weeklyHarvest),
...
//             {/* NEW ROW LAYOUT FOR HARVEST DURATION */}
//             <Row className="mb-3">
//                 <Col md={3}>
//                     <Form.Group>
...
//           <Row className="mb-4">
//             <Col>
//               <Alert variant={status.color} className="shadow-sm mb-0 p-4">
//                 {status.content}
//               </Alert>
//             </Col>
//           </Row>
...
//             <Col>
//               <Card className="p-3 shadow border-0">
//                 <h4 className="mb-1">📈 Historical Price Trends vs Predictions</h4>
//                 <p className="text-muted mb-3">Showing seasonal data for: <strong>{activeChartVeg}</strong></p>
//                 <ResponsiveContainer width="100%" height={300}>
...
//                     {/* Render the predicted crash point if it exists */}
//                     {isForecasting && financialImpact && financialImpact.isOversupplied && activeChartVeg === forecastForm.product && (
//                         <Line type="monotone" dataKey="PredictedCrash" name="⚠ Predicted Crash Price" stroke="#dc3545" strokeWidth={3} strokeDasharray="5 5" dot={{ r: 6, fill: '#dc3545' }} />
//                     )}
//                   </LineChart>
//                 </ResponsiveContainer>
//               </Card>
...
const Forecasting = () => {
  ...
  const handleClearForecast = () => {
    ...
    setChartVeg("All");
    setWeekOffset(0);
    setForecastForm({ product: "Carrot", area: "", units: "", cultivationStart: "", cultivationDays: "", cultivationEnd: "", growthDuration: "", harvestingStart: "", harvestingEnd: "", harvestDuration: "7", locationName: "Nuwara Eliya" });
    setIsDurationEditable(false);
  };
              ...
              <Card className="p-3 shadow border-0">
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={dynamicChartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    {/* Render Predicted Point Based on Market State */}
                    {isForecasting && financialImpact && activeChartVeg === forecastForm.product && (
                        <Line 
                            type="monotone" 
                            dataKey="PredictedPrice" 
                            name={`Predicted ${financialImpact.state === 'oversupply' ? 'Drop' : 'Spike'}`} 
                            stroke={financialImpact.state === 'oversupply' ? "#dc3545" : "#28a745"} 
                            strokeWidth={3} 
                            strokeDasharray="5 5" 
                            dot={{ r: 6, fill: financialImpact.state === 'oversupply' ? "#dc3545" : "#28a745" }} 
                        />
                    )}
                  </LineChart>
                </ResponsiveContainer>
